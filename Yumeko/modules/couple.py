...
OWNER_ID = 7392339658
SPECIAL_FEMALE_ID = 8107397304
SPECIAL_FEMALE_NAME = "𝐂ᴏʟᴅ 𝐂ᴏғғᴇᴇ"
OWNER_NAME = "𝐒ᴜɴɢ 𝐉ɪɴᴡᴏᴏ"
...
async def couple_handler(client: Client, message: Message):
    ...
    if message.from_user.id == OWNER_ID:
       ...
       # Check if the command is used by OWNER_ID
    if message.from_user.id == SPECIAL_FEMALE_ID:
        male_mention = f"[{message.from_user.first_name}](tg://user?id={SPECIAL_FEMALE_ID})"
        female_mention = f"[{OWNER_NAME}](tg://user?id={OWNER_ID})"
        await client.send_photo(
            chat_id,
            photo=img_url,
            caption=(
...
async def waifu_handler(client: Client, message: Message):
    ...
    # Owner and Special Female ID filter
    if user_id == OWNER_ID:
        waifu_mention = f"[{SPECIAL_FEMALE_NAME}](tg://user?id={SPECIAL_FEMALE_ID})"
        await message.reply_text(
            f"✨ **{user_mention}'s Today's Waifu** ✨\n"
            f"╭──────────────\n"
            f"┊•➢ {waifu_mention}\n"
            f"┊•➢ **Bond Percentage:** 100%\n"
            f"╰───•➢♡"
        )
        return

    if user_id == SPECIAL_FEMALE_ID:
        waifu_mention = f"[{OWNER_NAME}](tg://user?id={OWNER_ID})"
        await message.reply_text(
            f"✨ **{user_mention}'s Today's Waifu** ✨\n"
            f"╭──────────────\n"
            f"┊•➢ {waifu_mention}\n"
